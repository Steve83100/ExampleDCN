...
class Router(Switch):
    ...
    def start(self, controllers):
        ...
        self.waitOutput()
        self.cmd("/usr/lib/frr/bgpd -f ./test_conf/%s.conf -d -i /tmp/%s_bgpd.pid > ./test_log/%s_bgpd-stdout.log 2>&1" % (r, r, r), shell=True)
        # zebra and bgpd can share the same config file, or have their own config files. Both will do.
        self.waitOutput()
...
class MyTopo( Topo ):
    def build(self):
        ...
        h3 = self.addHost('h3', ip = "10.0.3.1/24", defaultRoute = "via 10.0.3.254")
